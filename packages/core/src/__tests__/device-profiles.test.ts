...
describe('Device profiles', () => {
  it.each(ALL_PROFILES.map((p) => [`${p.brand} ${p.model}`, p]))('%s has all required fields', (_label, profile) => {
  ...
  it.each(
    ALL_PROFILES.map((p) => [`${p.brand} ${p.model}`, p]),
  )('%s includes GATT HR service UUID', (_label, profile) => {
    const p = profile as DeviceProfile;
  ...
  it.each(
    ALL_PROFILES.map((p) => [`${p.brand} ${p.model}`, p]),
  )('%s includes heartRate capability', (_label, profile) => {
    const p = profile as DeviceProfile;
    expect(p.capabilities).toContain('heartRate');
  });

  it.each(
    BRAND_PROFILES.map((p) => [`${p.brand} ${p.model}`, p]),
  )('%s has a non-empty namePrefix', (_label, profile) => {
    const p = profile as DeviceProfile;
